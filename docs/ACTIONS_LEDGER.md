...
- **2026-06-23 — REVIEWED (no action needed): sick "come-at-X" is NOT an over-penalty.** Traced the sibling case:
  ...
  So unlike leave-early (which booked a full payback + −15), come-at-X has no immediate over-charge. Minor OPEN
  question only (murky, low stakes): whether a sick "come-at-X" then a late check-in is scored late_INFORMED
  (−1/min) vs uninformed (−2/min) — i.e. does telling us via the sick flow count as the declaration. Worth a
  look someday; not a clear bug. **Blast-radius of the leave-early bug VERIFIED CONTAINED: only Long's Jun-21
  (#51) — both −15 + payback fixed; every other own-sick since go-live was a genuine absence, correctly charged.**
