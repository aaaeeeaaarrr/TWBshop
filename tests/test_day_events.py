...
def _leave_early_minutes(p):
    return sorted(m for _off, m, label in staff_day_events(p) if label.startswith("leave-early"))
...
def test_day_shift_events_all_same_day():
    ...
    assert _leave_early_minutes(p) == [1090, 1100, 1120]   # nudges at +10/+20/+40
...
def test_overnight_shift_end_events_carry_next_day_offset():
    ...
    assert _leave_early_minutes({"work_start": "21:00", "work_end": "06:00"}) == [370, 380, 400]
