...
def test_digest_groups_mismatches_with_fixes_and_readiness():
    ...
    try:
        ...
        d = build_digest(ORG)
        assert d["stats"]["live_today"] == 3 and d["stats"]["live_all"] == 3
        assert d["stats"]["open_groups"] == 2          # two distinct LIVE patterns
...
def test_carryover_until_reconciled():
    ...
    try:
        ...
        d = build_digest(ORG)
        assert d["stats"]["live_today"] == 0            # nothing in the last 24h
        ...
        d2 = build_digest(ORG)
        assert d2["stats"]["open_count"] == 0 and "No open LIVE mismatches" in d2["text"]
    finally:
        _clean()
