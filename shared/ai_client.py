...
async def extract_receipt(image_bytes: bytes) -> dict:
    ...
    prompt = (
        ...
        "- date: the receipt/invoice date. Khmer receipts label it ថ្ងៃ (day) ខែ (month) ឆ្នាំ (year) "
        "— read the number written by each (a 2-digit year like '26' = 2026). Give your best YYYY-MM-DD "
        "from what is WRITTEN, even if the handwriting is messy. Use null ONLY when the date line is "
        "genuinely BLANK / not filled in (staff can correct a messy read)\n"
