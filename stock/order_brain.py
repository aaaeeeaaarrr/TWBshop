...
Migrated into the stock lane from gm_bot/stock.py (session 44 — C2). Identical logic; the GM copy
is removed at the integrator cutover (it is owner-test-gated, not live for staff). From each item's
minimum + current on-hand (and, once learned, its daily usage rate), decide what to order and how
much, and format the reorder message.

The order-quantity formula is a FIRST-PASS default (order back to min + a buffer, or enough to cover
lead-time usage when known). The owner tunes it on real output.
"""
...
def format_order_message(rows: list[dict]) -> str | None:
    """The reorder message. None if nothing needs ordering."""
    if not rows:
        return None
...
def usage_trend(recent_avg: float | None, older_avg: float | None,
    ...
    """'declining' if recent usage is >=30% below the older baseline (candidate off-menu / slowing
    item to flag to the owner), 'rising' if up a lot, else 'steady'. 'unknown' without enough history."""
