...
**Last updated:** 2026-06-26 (session 56 — **owner-shaped features + reliability cut-over: 7 live-gm settings now config-driven**).
...
**▶▶ RESUME HERE (after a /clear or compaction → owner types "keep going"): CONTINUE the tweakability rollout — extend `core.policy.SETTING_POLICY` responsibility one-liners to the OTHER domains' settings (stock · pos · hr_payroll · accountant — they auto-render via the generic `wizard/app.py::_field_input` wiring) + optionally build "ASK-TO-CHANGE" (NL "make lateness stricter" → applies a config tweak, on top of `core.ask`). EVERY other open/parked/dropped item → `docs/PENDING_WORK.md` (the master WRAP, kept comprehensive).**
