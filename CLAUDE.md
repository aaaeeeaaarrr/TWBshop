...
**Last updated:** 2026-06-21 (session 51 — **3 staff-data investigations (Heng/Chenda/Long) + guard read-only fix + event-log/anomaly-detector foundation**).
...
- **INVESTIGATION (read-only prod, all 3 confirmed):** **Heng** (id37) debt#148 owed96/paid89 — TWO real bugs: (a) a PHANTOM Jun-21 89-min slot (booking#62/sc#268) booked Jun19 20:08 BEFORE the Jun-19 89 credited (~23:00) → the bookable-remainder gate ignores a worked-but-uncredited same-day slot (stale-push hits the same gap); (b) the Jun-20 7-min TAIL (6:00–6:07, he checked out 06:07 = worked it) never credited. **Chenda** (id7) — CLEAN, owes nothing (59 cleared); only display/UX glitches (overnight-tail label "6am" under date 20/06 is really Sun 21 morning; missed checkout = left 31min before extended end). **Long** (id1) — paperless own-sick Jun19 declared 20:55 (5min pre-21:00 shift) → 540-min debt, BUT the −15 `late_sick_inform` DIDN'T FIRE (root: `_sick_late_mins`/`_shift_date_now` returns None in the 30-min PRE-shift window — same bug disables the FAMILY soft-note). early_arrival is +10/event (the "+1" was event-count; not a bug).
- **SHIPPED-INERT:** `gm_bot/events.py` append-only `gm_events` audit log (best-effort, never raises) `52112c3` · `audit.py::v_late_sick_penalty` precise −15-didn't-fire detector (wires into the live-watchdog → DMs owner; flags exactly Long on real data) `be74a00`.
- **REJECTED (important):** a `v_pb_uncredited` heuristic (sum done-bookings > paid) — it FALSE-POSITIVES: Nak/Chantrea came a few min late to come-early slots → correct PARTIAL credit, NOT a bug; only Heng (worked the full tail, 0 credit) is real. A noisy check is worse than none. Precise detection belongs in the credit-logic fix (#7) + per-event verify, not a static heuristic.
- **TAIL-CHASING design smell (owner decision):** tiny remainders (Nak owes 1min, system booked a 1-min payback slot for Jun21; Chantrea 3min) → re-booking 1 minute of come-early is absurd UX. Need a minimum-payback floor or sub-threshold forgiveness — owner call.
- **▶ STILL TO BUILD (HIGH-RISK live → staging proof + ONE batched quiet-window deploy):** ① shift-window fix (−15 own + family note + push offer) ② sick-reason mandatory-first (who→relationship-aware reason→confirm→FYI-at-confirm) ③ return-announcement fix + overnight-tail display (incl. Supervisors FYI) ④ Heng gate-fix + 7-min credit-fix ⑤ instrument `log_event` everywhere + `init_events_db` on startup ⑥ token-out-of-logs. **OWNER RUNS (vetted scripts, after deploy):** Heng data-fix (cancel phantom #62/#268 + credit the 7 → debt 0) · Long −15 + message + payback. **HELD:** both Supervisors group posts until all sorted. (ledger: `docs/ACTIONS_LEDGER.md` Open.)
- **FORENSICS GAP found:** GM bot logs only scheduler/HTTP/errors — NO per-staff events (sick/push/click/points/FYI). That's why "did the push send / did he click" is unanswerable today → #5 fixes it. Flaky F14 tests = pre-existing managed-PG connection flakiness under threaded load (OperationalError, count oscillates), NOT a regression (clean code passes them in 3.7s).
