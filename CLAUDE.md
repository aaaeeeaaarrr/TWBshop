...
**▶ LINEUP ADVANCED — computation core mirrored + parity-locked (commits `4dddcf2` + `06f36ff`; suite 903p/2s/0f).** **#2 points DONE** (`core/points.py`, parity vs `gm_bot.points`). **#3/#4/#5 settle MATH DONE** (`core/settle.py`: worked_minutes · ot_earned · split_ot_pb · settle_shift + honest bank-cap — DRIFT-GUARDED vs `gm_bot.ot` across a grid). So the platform now mirrors the WHOLE attendance computation (check-in verdict proven on real data + points + settle math), all SHADOW-ONLY/parity-locked, acting on nothing. **▶ REMAINING (the genuinely HIGH-RISK part = the live cut-over build, atomic-claim-first, fresh focus):** the settle ORCHESTRATION (which debt · redefine window · atomic claim-or-reject on the bank · over-book guard) · **#6 AL/sick math** · **#7 schedule-changes** (model as `shift_moved` events) · then web adapter + onboarding wizard + multi-tenancy + integrations → `docs/PLATFORM_ROADMAP.md`.
