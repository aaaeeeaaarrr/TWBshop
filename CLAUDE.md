...
- **▶ STEP 2 (durable handwriting fix) — IN BUILD → `docs/REPORT_SYSTEM_DESIGN.md §G`.** Feed each vendor's learned items + typical price bands *into* `extract_receipt` as soft context + a price "did-you-mean" ranking in the ✏️ Fix flow for low-confidence names. **Owner-locked:** (i) cold-read + post-read did-you-mean (no two-pass) · did-you-mean in Fix (card stays clean) · new-supplier = create-immediately behind a fuzzy dedup gate → **lean interim `needs_review` flag** (not the full Pending queue yet). **No new tables, no new API call** (priors = query over `acc_item_aliases` + `acc_receipt_lines`; ranking pure Python).
  ...
  - **FOOD MONEY (session 50): calc BUILT + inert, button integration PENDING owner decisions.** `gm_bot/food_money.py::food_money_cents` — 500៛/standard hour ÷4000 HALF-UP (9h→$1.13), standard hrs = scheduled shift length, no OT/PB, no-show→$0 (6 tests, nothing imports it, no deploy). Owner-preferred UX = a GM "Food allowance" button → pick on-shift staff → records a cash expense for that report period, name disappears (no double-give) → "Recorded for mid-day ~4pm / night ~5am". **⛔ DO NOT wire/deploy** until owner answers (ROADMAP §G): standard=scheduled? · report-window assignment · cash-expense tie-in (reports are typed, no itemized ledger) · menu location/who-presses · then HIGH-RISK live deploy. Checkout-only timing parked (owner will ask staff).
