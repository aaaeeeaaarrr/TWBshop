...
- **▶ NEXT = STEP 2 (the durable handwriting fix, being drafted):** feed each vendor's learned items + typical price bands *into* `extract_receipt` as soft context (read toward plausible, not cold) + a price/attribute "did-you-mean" ranking for low-confidence names. Guardrails locked: a ranked guess never beats a confident read · learn only from HUMAN corrections (never its own accepted guesses) · priors are soft hints, not anchors · prices are a signal, the receipt number stays truth.
**(prev, session 49)**
