...
**▶ SHADOW SELF-REPORTING BUILT (deploy next quiet window):** nightly digest DM (`_shadow_digest_job`, 21:45 PP) — carryover (a missed night combines into the next) + grouped mismatches + proposed fix per pattern + readiness; the live hook moved AFTER the staff reply (zero live latency). All gated by `shadow_run`. Committed `2c3a3e4`; deploy (gm) at the next non-shift-edge window.
