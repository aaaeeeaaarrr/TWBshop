...
**Last updated:** 2026-06-22 (session 52 — **PLATFORM BUILD STARTED: first slice = channel-agnostic multi-tenant check-in/out core (`core/`), shadow-run foundation, INERT**).
...
**▶ SHADOW SELF-REPORTING DEPLOYED + the verdict-parity port (tag `session-52c-verdict-parity-20260622` = `1501c13`, shadow on).** Nightly digest DM (`_shadow_digest_job` 21:45 PP — carryover + grouped mismatches + proposed fix + readiness) live; live hook moved AFTER the staff reply (zero latency). **PORT #1 DONE — `core.attendance.verdict` matches live (GRACE 5 / EARLY 5 / minute-of-day), per-tenant config; SHADOW-ONLY (live's `gm_bot/checkin` untouched).** **Replay loop PROVEN: 24% → 84% agree** (135 real check-ins, re-measured in seconds, confirmed on deployed server code). All gm restarts this morning clean (NR=0); shadow isolated — zero live interference. **▶ DIAGNOSIS NAILED (replay split by redefine-day, read-only):** **normal days ≈99% agree** once the ONE-TIME go-live-launch day (2026-06-16, a one-off grace) is excluded — the core verdict logic is CORRECT for the everyday case. The ONLY recurring gap = **REDEFINE days** (payback/OT slots moved the start; live judged vs the moved start, the core vs the base). So we're ONE well-defined port from cut-over readiness — days-not-weeks validated by real data.
**▶ NEXT (final) PORT: REDEFINE-AWARENESS** — make `core` resolve the day's moved start (a `shift_moved` in the entity model; for the shadow, feed/derive the resolved start like live's `resolve_day`). Design fork to decide first: core SELF-derives from its own shift_moved events (the clean platform way) vs is-TOLD the resolved start (quick shadow shortcut). Then re-replay (exclude the launch day) → expect ≈99%+ → READY. **Bigger port — do it as a FRESH focused session** (avoid more shift-edge gm restarts; today's are all clean NR=0). Shadow stays live + DMs the digest at 21:45 meanwhile.
