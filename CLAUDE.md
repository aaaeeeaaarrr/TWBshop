...
**Last updated:** 2026-06-01 (session 26 — lateness ladder, tagging, Lost>$2 ask, finance AI-fallback, weekly digest — all live)
...
- Tests: test_finance.py now 26 (added lost threshold + report-attempt heuristic + alias learning). Targeted GM suite 74/74. NOTE: `python -m pytest tests/` (whole dir) fails at collection due to the project secrets.py shadowing stdlib `secrets` across unrelated modules — run targeted files (pre-existing infra quirk, not from this work).
